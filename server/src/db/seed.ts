...
import { checkins, givebutterContacts, memberships, payments, students } from "./schema.js";
...
async function seed() {
  ...
  // 1. Active member, not checked in yet.
  const [ana] = await db
    .insert(students)
  ...
  // 2. One-time payer with 2 unredeemed credits (bought a pass for a friend) — this
  //    is the "let a friend use one" scenario from the spec.
  const [ben] = await db
    .insert(students)
  ...
  // 3. One-time payer, single credit.
  ...
  // 4. No payment on file at all — should flag red.
  await db.insert(students).values({ email: "dev@example.com", name: "Devon Diaz" });
  ...
  // 6. No payment on file — worst case, front desk override territory.
