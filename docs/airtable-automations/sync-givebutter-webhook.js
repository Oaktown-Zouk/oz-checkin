...
// independent script execution — nothing serializes them. The old version of
// this script used base.getTable(...).selectRecordsAsync() to check whether a
// Member with that Contact ID already existed, then createRecordAsync() if
// not. That's a read-then-write race with no lock between the two steps: two
// concurrent executions can both read "not found" and both create a row,
// producing two Members for one real person (this happened in production on
// 2026-09-02 — see this folder's README).
//
...
async function upsertAirtableRecord(tableId, fieldsToMergeOn, recordFields, attempt = 0) {
  ...
  // Same 5 req/sec-per-base limit the app server backs off on (see
  // server/src/airtable/realClient.ts) — this script now makes real REST
  // calls instead of Scripting-SDK calls, so it's subject to it too.
...
const toSelectField = (value) => { const text = toText(value); return text ? { name: text } : null; };
...
async function upsertMemberByContactId(contactId, firstName, lastName, email, phone) {
  ...
  if (existingMemberRecord) {
    ...
    if (toText(firstName) && !existingMemberRecord.getCellValueAsString('First Name')) {
      memberFields['First Name'] = toText(firstName);
    }
    if (toText(lastName) && !existingMemberRecord.getCellValueAsString('Last Name')) {
      memberFields['Last Name'] = toText(lastName);
    }
  } else {
    memberFields['First Name'] = toText(firstName);
    memberFields['Last Name'] = toText(lastName);
    memberFields['Email'] = toText(email).toLowerCase();
    memberFields['Phone'] = toText(phone);
  }
...
// run history for the actual error. (The previous version of this script
// never logged at all, which is part of what made the 2026-09-02 incident
// hard to diagnose — there was no record it had even run.)
...
if (eventType.startsWith('plan.')) {
  ...
  const coversMemberAlreadyAssigned = existingPlanRecord && (existingPlanRecord.getCellValue('Covers Member') ?? []).length > 0;
  if (memberRecordId && !coversMemberAlreadyAssigned) planFields['Covers Member'] = [{ id: memberRecordId }];
