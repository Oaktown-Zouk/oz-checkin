...
As part of a broader move away from a row-per-credit `Credits` table (see
`docs/airtable-schema.md`'s "Credits" section), this script no longer creates
`Credits` rows or resolves the transaction's `Member` at all — it just sets
`"Credits Purchased"` (a number) on the triggering `Transactions` record itself, and
a `Members` rollup picks it up automatically. Since the only reason this script ever
needed the `Member` link was to attach the `Credits` rows it created to the right
person, removing that write also removes the lookup — which, incidentally, fully
retires the entire class of bug this script had earlier (a `memberIds` input
...
below), and Airtable's REST API wants a select field as a **plain string** — it
...
**Required manual setup before pasting these in:** a `Recurring Plans` field on
`Transactions` (Link to another record → `Recurring Plans`), created by hand —
automations can't add fields. Backfilling every already-synced transaction (not just
future ones) required temporarily setting `sync-givebutter-transactions.js`'s
`LOOKBACK_DAYS` to `3650`, running it once manually, then setting it back to `7`
before re-enabling the schedule — the same backfill mechanism the script's own
header documents for the initial historical pull.
...
(This script was later rewritten entirely to drop the `Credits` table and the
`Member` lookup along with it — see "no more Member lookup, no more Credits table"
above — so this specific bug can no longer recur.)
...
- The same-day pair (Contact ID 44573119, both rows created at the same second,
  ...
  *link*, it doesn't stop the second row from being created in the first place, and
  ...
  Member id instead of splitting across two. This needed a dedicated Airtable Personal
